...
module.exports.fetchNews = async (req, res) => {
  try {
    ...
    const articles = isFull ? await getAllNews({ q: q.trim(), language, country }) : await getLatestNews({ q: q.trim(), language, country });
    res.json({ articles , total: articles.length,});
  } catch (err) {
    console.error("[fetchNews]", err);
    res.status(500).json({articles: [] , total: 0 , message: err instanceof Error ? err.message : "Không thể tải tin tức",
    });
  }
};